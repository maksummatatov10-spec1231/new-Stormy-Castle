...
def make_open_method(p: Pool, q: dict, strings: dict, colors: dict) -> bytes:
    a = Assembler()
    # locals: 0=this, 1=Event, 2=panel, 3=temp, 4=input
    a = Assembler()
    ...
    a.getlocal(2); a.getlex(q['Math']); a.getlex(q['stage']); a.pool(0x66, q['mouseX']); a.push_number(590); a.emit(0x46); a.code += u30(q['min']) + u30(2); a.pool(0x61, q['x'])
    a.getlocal(2); a.getlex(q['Math']); a.getlex(q['stage']); a.pool(0x66, q['mouseY']); a.push_number(425); a.emit(0x46); a.code += u30(q['min']) + u30(2); a.pool(0x61, q['y'])
    ...
    a.getlocal(2); a.pool(0x66, q['graphics']); a.push_number(0); a.push_number(0); a.push_number(210); a.push_number(175); a.emit(0x4f); a.code += u30(q['drawRect']) + u30(4)
    ...
    a.getlocal(3); a.pool(0x66, q['graphics']); a.push_number(0); a.push_number(0); a.push_number(210); a.push_number(27); a.emit(0x4f); a.code += u30(q['drawRect']) + u30(4)
    ...
    a.getlocal(3); a.push_number(190); a.pool(0x61, q['width'])
    ...
    def add_button(name_key: str, caption_key: str, color_key: str, xx: int, yy: int) -> None:
        a.pool(0x5d, q['Sprite']); a.emit(0x4a); a.code += u30(q['Sprite']) + u30(0); a.emit(0xd7)
        ...
        a.getlocal(3); a.pool(0x66, q['graphics']); a.push_number(0); a.push_number(0); a.push_number(90); a.push_number(27); a.emit(0x4f); a.code += u30(q['drawRect']) + u30(4)
        a.getlocal(3); a.pool(0x66, q['graphics']); a.emit(0x4f); a.code += u30(q['endFill']) + u30(0)
        a.getlocal(3); a.emit(0x27); a.pool(0x61, q['mouseChildren'])
        ...
        a.getlocal(5); a.push_number(80); a.pool(0x61, q['width'])
    ...
    a.getlocal(3); a.push_number(190); a.pool(0x61, q['width'])
    a.getlocal(3); a.push_number(25); a.pool(0x61, q['height'])
    a.getlocal(3); a.emit(0x27); a.pool(0x61, q['selectable'])
    a.getlocal(2); a.getlocal(3); a.emit(0x4f); a.code += u30(q['addChild']) + u30(1)
...
def make_event_method(p: Pool, q: dict, strings: dict, colors: dict) -> bytes:
    a = Assembler()
    # locals: 0=this, 1=Event, 2=target, 3=target name, 4=number/panel
    a.getlocal(0); a.emit(0x30)
...
def patch_abc(original: bytes) -> bytes:
    ...
    q = {
        ...
        'stage': 3590, 'MouseEvent': 3, 'CLICK': 3622,
    ...
    strings = {key: p.string(value) for key, value in {
        'rightClick': 'rightClick', 'fpsPanel': 'fpsPanel', 'fpsHeader': 'fpsHeader',
        'fpsInput': 'fpsInput', 'fps75': 'fps75', 'fps120': 'fps120',
        'fpsApply': 'fpsApply', 'fpsClose': 'fpsClose', 'defaultFps': '75',
        'input': 'input', 'title': 'Stormy Castle FPS', 'caption75': '75 FPS',
        'caption120': '120 FPS', 'captionApply': 'Apply', 'captionClose': 'Close',
        'footer': 'Right click to open; drag the header.',
    }.items()}
    colors = {
    ...
    add_method(abc, 0, q['fpsEventName'], event_code, maxstack=5, locals_=5)

    # The main class is Main (instance 0). Add stage listeners to the existing
    # startGame body, immediately before its return.
    setup = Assembler()
    # stage.addEventListener("rightClick", this.openFpsMenu)
    setup.getlex(q['stage']); setup.pool(0x2c, strings['rightClick']); setup.getlocal(0); setup.pool(0x66, q['fpsOpenName']); setup.emit(0x4f); setup.code += u30(q['addEventListener']) + u30(2)
